...
class CreateVector(smach.State):
    ...
    def execute(baseX, baseY, baseZ, tipX, tipY, tipZ):
        ...
        xzangle = getAngle(baseX, baseZ, tipX, tipZ)
        obj_xzangle = getAngle(baseX, baseZ, objX, objZ);
...
class FindBoundingBoxes(smach.State):
    ...
    def execute(self, userdata):
        ...
        objects = bboxes.bounding_boxes.markers[:]
        # track the smallest angular difference between the pointing vector
        # and a a vector from the finger base to the object
        ...
        for obj in objects:
            ...
            xzangle = getAngle(baseX, baseZ, tipX, tipZ)
            obj_xzangle = getAngle(baseX, baseZ, objX, objZ)
            diff = getAngleDiff(xzangle, obj_xzangle)
